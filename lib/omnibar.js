...
module.exports = class Omnibar {
    constructor(){
        ...
        this.refs.search.addEventListener('focus', this.focused);
        this.refs.search.addEventListener('blur', this.blurred);
        this.refs.search.addEventListener('input', this.didChangeQuery);

        this.disposables.add(new Disposable(() => {
            this.refs.search.removeEventListener('focus', this.focused);
            this.refs.search.removeEventListener('blur', this.blurred);
        ...
        this.disposables.add(via.symbols.onDidAddSymbol(this.loadDefaultItems));
        this.disposables.add(via.symbols.onDidRemoveSymbol(this.loadDefaultItems));
        ...
        return $.div({classList: 'omnibar'},
            $.div({classList: 'omnibar-input'},
                ...
                $.input({
                    ...
                    ref: 'search',
                    placeholder: this.scope.placeholder ? this.scope.placeholder : 'Enter a Symbol or Command...'
                }, 'via'),
    ...
    loadDefaultItems(){
        //Load keybindings
        const keybindings = via.keymaps.findKeyBindings({target: via.views.getView(via.workspace)});

        //Commands
        const commands = via.commands.findCommands({target: via.views.getView(via.workspace)}).map(command => {
            const keybinding = keybindings.find(kbd => kbd.command === command.name);
            return {group: 'commands', name: command.displayName, description: command.description, keybinding};
        });

        //Symbols
        const symbols = via.symbols.getSymbols().map(symbol => {
            // const keybinding = keybindings.find(kbd => kbd.command === command.name);
            return {group: 'symbols', name: symbol.identifier, description: symbol.description, symbol};
        });
        ...
        const ui = via.workspace.getOpenerConfigs().map(config => {
            ...
            return {group: 'interface', name: config.name, description: config.description, keybinding};
        });

        //Eventually news stories, etc.
        this.defaultItems = [].concat(commands, symbols, ui).sort((a, b) => a.name.localeCompare(b.name));
    ...
    renderItems(){
        ...
        let defaultGroup = [];
        let children = [];
        let max = this.scope.maxResultsPerCategory || Infinity;
        ...
        for(let entry of defaultGroup.slice(0, max)){
        ...
        for(let [group, entries] of Object.entries(groups)){
            ...
            for(let entry of entries.slice(0, max)){
                children.push(this.renderEntry(entry));
            }
        }
    ...
    renderEntry(entry){
        ...
        return $.li({classList: 'entry ' + (this.getSelectedItem() === entry ? 'selected' : '')},
