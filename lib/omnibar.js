...
module.exports = class Omnibar {
        ...
        return {
            ...
            didConfirmSelection: selection => {
                if(selection.command){
                    ...
                    via.workspace.open(`via://charts/${selection.symbol.identifier}`, {});
    ...
    loadDefaultItems(){
        ...
        const symbols = via.symbols.getSymbols().map(symbol => {
            // const keybinding = keybindings.find(kbd => kbd.command === command.name);
            return {group: 'symbols', name: symbol.name, description: symbol.description, symbol};
        });
    ...
    renderEntry(entry, index){
        ...
        return $.li({classList: 'entry ' + (this.getSelectedItem() === entry ? 'selected' : '')}, //, onClick: () => this.didClickItem(entry)
